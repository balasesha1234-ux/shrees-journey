...
const DEFAULT_ITEMS = ['2023', '2024', '2025', '2026'];
...
export const OptionWheel: React.FC<OptionWheelProps> = ({
  ...
  const initialIdx = selected !== undefined ? selected : defaultSelected;
  const posRef = useRef(initialIdx);
  const targetRef = useRef(initialIdx);
  const rafRef = useRef<number | null>(null);
  const lastRef = useRef(0);
  const cfgRef = useRef<Record<string, any>>({});
  const onChangeRef = useRef(onChange);
  const selectedRef = useRef(initialIdx);
  ...
  // Sync with controlled `selected` prop from external scroll triggers
  useEffect(() => {
