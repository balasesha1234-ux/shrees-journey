...
const getFontValue = (v: number | string) => (typeof v === 'number' ? `${v}px` : v);

const measureLine = (ctx: CanvasRenderingContext2D, line: string, ls: number) => {
  const chars = Array.from(line);
  return chars.reduce((w, c) => w + ctx.measureText(c).width, 0) + Math.max(0, chars.length - 1) * ls;
};

const drawLine = (ctx: CanvasRenderingContext2D, line: string, x: number, y: number, ls: number) => {
  const chars = Array.from(line);
  let cursor = x - measureLine(ctx, line, ls) / 2;
  chars.forEach((c, i) => {
    ctx.fillText(c, cursor, y);
    cursor += ctx.measureText(c).width + (i === chars.length - 1 ? 0 : ls);
  });
};
...
interface BuildOpts {
  ...
  height: number;
  dpr: number;
  props: Required<WarpTextProps>;
}

const buildTextCanvas = ({ container, width, height, dpr, props }: BuildOpts): HTMLCanvasElement => {
  ...
  // Measure real computed styles via a probe element
  ...
  const blockH = Math.max(lineHeight * lines.length, 1);
  const fit = Math.min(1, (width * 0.86) / widest, (height * 0.78) / blockH);
  if (fit < 1) { fontSizePx *= fit; letterSpacing *= fit; lineHeight *= fit; applyFont(); }

  const startY = height / 2 - (lineHeight * (lines.length - 1)) / 2;
  lines.forEach((line, i) => drawLine(ctx, line, width / 2, startY + i * lineHeight, letterSpacing));
  return canvas;
};
...
const WarpText: React.FC<WarpTextProps> = ({
  ...
  const propsRef = useRef<Required<WarpTextProps>>({
    text: text!, color: color!, warpStrength: warpStrength!, warpScale: warpScale!, speed: speed!,
    pointerInfluence: pointerInfluence!, pointerStrength: pointerStrength!, refraction: refraction!, ripple: ripple!,
    fontSize: fontSize!, fontWeight: fontWeight!, fontFamily: fontFamily!, letterSpacing: letterSpacing!, lineHeight: lineHeight!,
    className: className!, style: style!,
  });
  const contextRef = useRef<{ program: any; rasterize: () => void } | null>(null);
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn('WarpText: WebGL init failed.', e);
      return;
    }
    ...
    const renderOnce = () => { if (!disposed && !contextLost) renderer.render({ scene: mesh }); };

    const rasterize = async () => {
      const v = ++rasterVersion;
      try { await document.fonts?.ready; } catch {}
      if (disposed || contextLost || v !== rasterVersion) return;
      const rect = container.getBoundingClientRect();
      if (rect.width <= 0 || rect.height <= 0) return;
      const dpr = Math.min(window.devicePixelRatio || 1, 2);
      const tc = buildTextCanvas({ container, width: rect.width, height: rect.height, dpr, props: propsRef.current });
      texture.image = tc;
    ...
    const onContextLost = (e: Event) => { e.preventDefault(); contextLost = true; if (raf) cancelAnimationFrame(raf); raf = 0; };
    const onVisibility = () => {
      pageVisible = !document.hidden;
      if (pageVisible && visible && !raf) raf = requestAnimationFrame(loop);
      else if (!pageVisible && raf) { cancelAnimationFrame(raf); raf = 0; }
    };
    ...
    const loop = (now: number) => {
      ...
      const tx = pointer.activeTarget > 0 ? pointer.tx : idleX;
      const ty = pointer.activeTarget > 0 ? pointer.ty : idleY;
