...
export const TimelineScene: React.FC = () => {
  ...
  return (
    <section
      ref={containerRef}
      id="timeline-section"
      className="relative w-full min-h-screen py-24 bg-[#0B0B0F] text-[#f0f0f5] font-general select-none overflow-hidden"
    >
      {/* 3D OPTION WHEEL PINNED ROTARY NAVIGATION */}
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-16 pl-6 md:pl-56 lg:pl-64 flex flex-col gap-40 md:gap-56 relative z-10">
          ...
          return (
            ...
            >
              ...
              >
                ...
                <div className="absolute left-[-2rem] top-[-3rem] pointer-events-none select-none font-general text-[10rem] sm:text-[16rem] lg:text-[22rem] font-black text-white/[0.02] leading-none z-0">
                  {spread.year}
                </div>

                {/* Header Bar */}
                <header className="relative z-10 animate-reveal flex flex-wrap items-center justify-between border-b border-white/10 pb-6 gap-4">
                  ...
                  <div className="flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#0c0d12]/90 border border-white/10 shadow-lg">
                    <Calendar className="w-3.5 h-3.5 text-[#e5c158]" />
                    <span className="font-general text-[11px] font-semibold tracking-wider text-[#f0f0f5]/80 uppercase">
                      {spread.badge}
                    </span>
                  </div>
                </header>
                ...
                <div
                  className={`relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center mt-8 ${
                ...
                >
                  ...
                  <div className={`lg:col-span-5 flex flex-col gap-6 ${isEven ? 'lg:order-1' : 'lg:order-2'}`}>
                    ...
                    <div className="animate-reveal flex flex-col gap-3">
                      <BlurText
                        ...
                        animateBy="words"
                        direction="top"
                        className="font-general text-3xl sm:text-5xl font-extrabold tracking-tight text-[#f0f0f5] leading-tight"
                      />

                      <div className="border-l-2 border-[#e5c158] pl-4 mt-2">
                        <BlurText
                          text={spread.quote}
                          delay={100}
                          animateBy="words"
                          direction="bottom"
                          className="font-general italic text-base sm:text-lg text-[#e5c158]/90 leading-relaxed"
                        />
                      </div>
                    </div>

                    <ul className="animate-reveal flex flex-col gap-4 my-2 text-sm sm:text-base text-[#f0f0f5]/80 leading-relaxed font-normal">
                      {spread.bullets.map((b, idx) => (
                        <li key={idx} className="flex items-start gap-3">
                          <span className="w-1.5 h-1.5 rounded-full bg-[#e5c158] mt-2 shrink-0 shadow-[0_0_8px_#e5c158]" />
                          <span>{b}</span>
                        </li>
                      ))}
                    </ul>
                    ...
                    {spread.milestones && (
                      <div className="animate-reveal flex flex-wrap gap-2.5 mt-2">
                        {spread.milestones.map((ms, idx) => (
                          <div
                            key={idx}
                            className="flex items-center gap-2 px-3.5 py-1.5 rounded-xl bg-[#e5c158]/10 border border-[#e5c158]/30 text-xs font-semibold uppercase tracking-wider text-[#e5c158]"
                          >
                  ...
                  <div className={`lg:col-span-7 ${isEven ? 'lg:order-2' : 'lg:order-1'}`}>
                    <div className="animate-reveal relative w-full h-[520px] sm:h-[600px] rounded-3xl overflow-hidden border border-white/10 shadow-2xl">
                      <DriftWall
